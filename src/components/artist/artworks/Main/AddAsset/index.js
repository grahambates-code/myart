...
const ADD = gql`
         ...
         mutation ($artwork_id : uuid!, $data : String) {
          insert_asset_table_one(object: {artwork_id: $artwork_id, data: $data, name: "Star", type: "SVG"}) {
            id
          }
        }
    `;
...
export default function AddAsset({ refetch, artwork, setSelectedArtwork }) {

    return (
        <Mutation
            mutation={ADD}
            variables={{artwork_id : artwork.id, data : SVG}}
            onCompleted={(e) => {
                refetch && refetch();
